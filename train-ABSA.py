#! -*- coding:utf-8 -*-
#参考   https://www.biendata.xyz/competition/sohu_2022/
...
# 配置设置/data/xinjiesun/ExpCode/Code_Sun/BERT-ABSA-SUN/bert-large-uncased
config_path = '/data/xinjiesun/ExpCode/Code_Sun/BERT-ABSA-SUN/bert-large-uncased/config.json'
checkpoint_path = '/data/xinjiesun/ExpCode/Code_Sun/BERT-ABSA-SUN/bert-large-uncased/pytorch_model.bin'
dict_path = '/data/xinjiesun/ExpCode/Code_Sun/BERT-ABSA-SUN/bert-large-uncased/vocab.txt'
data_dir = '../Sohu2022_data/nlp_data'
...
def load_data(filename):
    ...
    with open(filename, encoding='utf-8') as f:
        for l in tqdm(f.readlines(), desc="Loading data"):
            ...
            text2_len = sum([len(ent.split()) + 1 for ent in taskData['entity'].keys()]) + 1
            # for t in text_segmentate(taskData['content'], maxlen - text2_len - 2, seps, strips):
            D.append((taskData['content'].strip(), text2, taskData['entity']))
    return D
...
def collate_fn(batch):
    ...
    for text1, text2, entity in batch:
        ...
        # print(f"len(token_ids1)={len(token_ids1)}")
        tokens = tokenizer.decode(token_ids1)
        tokens2 = tokenizer.tokenize(text2)[1:-1]
        # # 手动处理拆分后的结果
        # final_tokens2 = []
        # for token in tokens2:
        #     if token.startswith('##'):
        #         final_tokens2[-1] += token[2:]
        #     else:
        #         final_tokens2.append(token)
...
# 转换数据集
# all_data_train = load_data(f'{data_dir}/Laptops_Train_output--no[cls] - 让GPT生成一些0和-1的样本进行增强.txt')
# all_data_test = load_data(f'{data_dir}/Laptops_Test_output--no[cls].txt')

# all_data_train = load_data(f'{data_dir}/Twitter_Train_output-no[cls].txt')
# all_data_test = load_data(f'{data_dir}/Twitter_Test_output-no[cls].txt')

all_data_train = load_data(f'{data_dir}/Restaurants_Train_output.txt')
all_data_test = load_data(f'{data_dir}/Restaurants_Test_output.txt')


# all_data=all_data[:1000]
split_index = int(len(all_data_train) * 0.8)
...
train_dataloader = DataLoader(ListDataset(data=all_data_train[:split_index]), batch_size=batch_size,shuffle=True, collate_fn=collate_fn)

valid_dataloader = DataLoader(ListDataset(data=all_data_train[split_index:]), batch_size=batch_size_eval, shuffle=True,collate_fn=collate_fn)

test_dataloader = DataLoader(ListDataset(data=all_data_test), batch_size=batch_size_test, shuffle=True,collate_fn=collate_fn)

valid_dataloader=test_dataloader   #验证集=测试集
# 定义bert上的模型结构
class Model(BaseModel):
    ...
    def forward(self, *inputs):
        ...
        entity_states = self.dropout(entity_states)
...
class_weights = torch.tensor([0.25, 0.25,0.5])  # 根据实际情况设置类别权重
# # 权重调整
...
#     def __init__(self, alpha=0.25, gamma=2, reduce=True, ignore_index=-100):
...
#             return F_loss
# class FocalLoss(nn.Module):
#     def __init__(self, alpha=None, gamma=3, reduce=True, ignore_index=-100):
#         super(FocalLoss, self).__init__()
#         self.alpha = alpha if alpha is not None else torch.tensor([2, 2, 2])
    ...
    def evaluate(data):
        ...
        for (token_ids, entity_ids), entity_labels in tqdm(data):
            ...
            if use_swa:
                swa_model.eval()
                with torch.no_grad():
                    entity_logit = F.softmax(swa_model([token_ids, 1]), dim=-1)  # [btz, 实体个数, 实体类别数]
            else:
...
if __name__ == '__main__':
    if choice == 'train':
        evaluator = Evaluator()
        # adversarial_train = AdversarialTraining('fgm') #对抗训练
        # model.fit(train_dataloader, epochs=epochs, steps_per_epoch=steps_per_epoch,
        #           callbacks=[evaluator, adversarial_train])
        model.fit(train_dataloader, epochs=epochs, steps_per_epoch=steps_per_epoch,
                            callbacks=[evaluator])

    model.load_weights(ckpt_path)
    f1, acc, pred_result = Evaluator.evaluate(valid_dataloader)
    f11, acc1, pred_result1 = Evaluator.evaluate_test(test_dataloader)
